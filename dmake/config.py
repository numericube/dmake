...
class Config(base_commands.BaseCommand):
    ...
    def container_to_docker_compose(self, container_id):
        ...
        container_info = json.loads(self.system("docker inspect {}".format(container_id), capture=True))[0]
        image_info = json.loads(self.system("docker inspect {}".format(container_info["Image"]), capture=True))[0]
        ...
        for first_level_key in (
            ...
            for content_key, content_value in content.get(first_level_key, {}).items():
                if not content_key in compose[first_level_key]:
                    ...
                    printc(bcolors.INFO, "  Create '{}:{}'".format(first_level_key, content_key))
                else:
                    printc(bcolors.INFO, "  Update '{}:{}'".format(first_level_key, content_key))
    ...
    def setup_compose(self):
        ...
        # Setup services
        services = {"services": {}}
        container_ids = self.system("docker ps --format '{{.ID}}'", capture=True).split()
